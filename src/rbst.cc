...
public:
  RBST(): engine(std::random_device()()) {}
  ~RBST() {}
  size_type size() const { return size(root); }
  void push_back(T v) { root = merge(root, node(v)); }
  T at(size_type pos) const { return at(root, pos); }
private:
  template <class... Args>
  NodePtr node(Args&&... args) const
  {
    return std::make_shared<Node>(std::forward<Args>(args)...);
  }
  size_type size(const NodePtr& x) const { return x ? x->size_ : 0; }
  T at(const NodePtr& x, size_type pos) const
  {
  ...
  const NodePtr& propagate(const NodePtr& x) const
  {
    if (!x) return x;
    if (true) return x;
    if (x->left_) {
  ...
  NodePtr merge(const NodePtr& x, const NodePtr& y)
  {
    ...
    if (engine() % (size(x) + size(y)) < size(x)) {
      NodePtr newx = node(*x);
  ...
  NodePtrPair split(const NodePtr& x, size_type pos) const
  {
